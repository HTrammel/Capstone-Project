...
setup_source <- function() {
	tmp_df <- NULL
	tmp_df <- data.frame(cbind(tmp_df, src_type = c("blog", "news", "twitter")), stringsAsFactors = F)
	tmp_df <- cbind(tmp_df, src_file = c("Data/final/en_US/en_US.blogs.txt", "Data/final/en_US/en_US.news.txt",
		"Data/final/en_US/en_US.twitter.txt"), stringsAsFactors = F)
	tmp_df <- cbind(tmp_df, src_name = c("en_blog", "en_news", "en_twit"), stringsAsFactors = F)
	tmp_df <- cbind(tmp_df, src_lines = c(899288L, 1010242L, 2360148L))
	tmp_df <- cbind(tmp_df, src_rds = c("Data/USblogs.Rds", "Data/USnews.Rds", "Data/UStwitter.Rds"),
		stringsAsFactors = F)
	return(tmp_df)
}
...
get_data <- function(df, stp) {
	...
	tmp <- scan(con, what = "complex", nlines = maxlines, skip = maxlines * stp, fileEncoding = "UTF-16LE",
...
load_text <- function(flag) {
	if (flag == TRUE) {
		...
		file.remove(list = c("Data/voc_corpus.Rds", "Data/vocab.Rds", "Data/voc_ngrams.Rds",
...
make_vocab <- function(lines) {
	...
	v <- tokenize(lines, what = "word", verbose = TRUE, simplify = FALSE, removeSeparators = TRUE,
...
make_ngrams <- function(lines) {
	...
	if (DEBUG == TRUE)
		cat("Making tokens\n")
	n <- tokenize(lines, what = "word", verbose = TRUE, simplify = FALSE, removePunct = TRUE,
...
if (!file.exists("Data/vocab_df.Rds")) {
	...
	vocab_df <- vocab_df %>% mutate(rel_freq = vocab_freq/vocab_tot)
	...
	rm(list = c("vocab_freq", "vocab_tot", "vocab_term", "id", "en_all","vocab_dfm"))
...
if (!file.exists("Data/ngrams_df.Rds")) {
	...
	ngrams_dfm <- dfm(voc_ngrams, ignoredFeatures = stopwords("english"))
	...
	ngrams_df <- ngrams_df %>% mutate(rel_freq = ngram_freq/ngram_tot) %>% mutate(ng_id = ngram_name)
